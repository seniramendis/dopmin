...
// ─── NAV (Blog) ─────────────────────────────────────────────────────────────
// Same floating-pill nav template used on Solutions/Expertise/Work, with
// "Blog" set as the active link. Lives in its own file (rather than inline
// in the page, like the other templates do) because both blog pages are
// async server components and can't host client-only hooks directly.
      ...
      >
        <div className="px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8" aria-label="Main navigation">
            ...
            <Link href="/blog" className="text-[14px] font-medium text-[#F26A10] hover:text-[#D94030] transition-colors">Blog</Link>
          ...
          {open && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="md:hidden overflow-hidden border-t border-stone-100 rounded-b-2xl">
              <div className="px-6 py-5 flex flex-col gap-4">
                ...
                <Link href="/blog" onClick={() => setOpen(false)} className="text-[#F26A10] text-base font-semibold">Blog</Link>
