...
import { 
  ShoppingCart, ShieldCheck, Heart, Zap,
...
export default function ForttuneApp() {
  ...
  const updateCartQty = (id: string, qty: number) => {
    setCart(prev => prev
      .map(x => x.id === id ? { ...x, qty: Math.max(1, Math.min(x.stock ?? qty, qty)) } : x)
    );
  };

  // Adds the product straight to cart and jumps into checkout immediately
              ...
              <div className="w-full md:w-1/2 p-8 lg:p-10 flex flex-col">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex items-center justify-between border border-[#0D1B3E]/15 rounded-xl px-4 h-[48px] w-full sm:w-[104px] shrink-0 bg-white">
                    ...
                    <div className="flex gap-3 flex-1 h-[48px]">
                      <AddToCartButton
                      ...
                      />
                      <button
                        onClick={() => buyNow(selectedProduct, pdpQty)}
                        disabled={selectedProduct.stock === 0}
                        className="relative flex-1 rounded-xl font-semibold text-sm border-2 border-[#0D1B3E] text-[#0D1B3E] hover:bg-[#0D1B3E] hover:text-white transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-[#0D1B3E]"
                      >
                        <Zap size={16} /> Buy Now
                      </button>
                    </div>
                ...
                <div className="space-y-3">
                  {cart.map((x, i) => (
                    <div key={i} className="flex gap-3 p-3 bg-white border border-[#0D1B3E]/8 rounded-xl relative">
                      <div className="bg-[#F5F6FA] rounded-xl w-16 h-16 flex items-center justify-center shrink-0 overflow-hidden">
                        {x.image ? (
                          <img src={x.image} alt={x.name} className="w-full h-full object-contain mix-blend-multiply p-1" />
                        ) : (
                          getCatIcon(x.category, "text-[#1A2F5E]/40", 22)
                        )}
                      </div>
                      <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5">
                        <div className="text-sm font-semibold text-[#0D1B3E] truncate pr-5">{x.name}</div>
                        <div className="text-xs text-[#6B7A99]">{formatLKR(x.price)} each</div>
                        <div className="flex items-center justify-between mt-0.5">
                          <div className="flex items-center gap-2 border border-[#0D1B3E]/15 rounded-lg px-2 py-1 bg-white">
                            <button
                              onClick={() => x.qty <= 1 ? removeFromCart(x.id) : updateCartQty(x.id, x.qty - 1)}
                              className="text-[#6B7A99] hover:text-[#E85D26] transition-colors"
                            >
                              <Minus size={12} />
                            </button>
                            <span className="text-xs font-bold text-[#0D1B3E] w-4 text-center">{x.qty}</span>
                            <button
                              onClick={() => updateCartQty(x.id, x.qty + 1)}
                              disabled={typeof x.stock === 'number' && x.qty >= x.stock}
                              className="text-[#6B7A99] hover:text-[#E85D26] transition-colors disabled:opacity-30"
                            >
                              <Plus size={12} />
                            </button>
                          </div>
